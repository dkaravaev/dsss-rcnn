...
class ModulusMaximaPool1D(KL.MaxPool1D):
    def __init__(self, pool_size=2, strides=None,
                 padding='valid', data_format='channels_last', **kwargs):
        super(KL.MaxPooling1D, self).__init__(pool_size, strides,
                                              padding, data_format,
                                              **kwargs)

    def _pooling_function(self, inputs, pool_size, strides,
                          padding, data_format):
        E = KB.sum(inputs ** 2, axis=-1, keepdims=True)
        x = KB.concatenate([inputs, KB.sqrt(E)], axis=-1)
        output = KB.pool2d(x, pool_size, strides, padding, data_format, pool_mode='max')
        return output[:, :, :, :-1]


class EnergyMaximaPool1D(KL.MaxPool1D):
    def __init__(self, pool_size=2, strides=None,
                 padding='valid', data_format='channels_last', **kwargs):
        super(KL.MaxPooling1D, self).__init__(pool_size, strides,
                                              padding, data_format,
    ...
    def _pooling_function(self, inputs, pool_size, strides,
                          padding, data_format):
        E = KB.sum(inputs ** 2, axis=-1, keepdims=True)
        x = KB.concatenate([inputs, E], axis=-1)
        output = KB.pool2d(x, pool_size, strides, padding, data_format, pool_mode='max')
        return output[:, :, :, :-1]
